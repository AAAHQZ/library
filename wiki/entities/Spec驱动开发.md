...
source: 你以为在写Spec其实在绕路写代码, 告别氛围编程基于Harness治理和SDD的团队级AI研发范式演进与实践, 自然语言不适合编程Dijkstra半个世纪前的判断今天依然成立
---
...
## Docker Dijkstra 界面宽度理论基础

Dijkstra 的界面宽度理论（EWD 667, 1978）为 SDD 提供了更深层的理论依据：

- **SDD 通过在指令端重建形式化契约（Spec），本质是在收窄人机界面**
- 传统编程中，程序员用形式语言（C、Python、Haskell）直接表达精确意图，界面天生是窄的
- AI 编程中，人类用自然语言描述意图（宽界面），SDD 通过 Spec 将宽界面重新收窄
- 引用 Dijkstra：「跨界面协作会产生额外工作量，界面越宽，两侧越忙」——SDD 正是为了减少这种额外工作量

### Spec 作为窄接口

| 维度 | 无 Spec（宽界面） | 有 Spec（窄界面） |
|------|-----------------|------------------|
| 输入 | 模糊需求描述 | 结构化规格文档 |
| 歧义度 | 高 | 低 |
| 迭代成本 | 多轮拉扯 | 一次对齐 |
...
SDD 追求的「Spec 足够精确以至于接近代码」，本质上就是用形式符号系统替换自然语言系统的又一次尝试——这正是 Vieta、Descartes、Leibniz、Boole 等人开创的道路在 AI 时代的延续。
